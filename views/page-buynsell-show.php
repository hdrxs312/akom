...
<!-- START FILE -->
	<div class="edge d-flex w-100 mb-3">
		<a href="<?php e(home().$module) ?>" class="btn btn-sm btn-light border-secondary">
			<i class="fas fa-level-up-alt mr-2"></i> Jual Beli
		</a>
	</div>

	<h1 class="edge"><?php e($item['bs_title']) ?></h1>
	<div class="kard">
		...
		<div class="photo">
			<img src="<?php files($item['co_id'], 'bs_'.$item['bs_id'].'.jpg') ?>" style="height:120px">
		</div>
		<div class="body">
			<p><button class="btn btn-sm bg-light btn-outline-primary"><?php e($item['bs_price']) ?></button></p>
			<p><?php echo nl2br($item['bs_description']) ?></p>
		</div>
	</div>
	<div class="kard">
	<?php
		...
		<div class="body d-flex mb-3">
			...
			<div class="right w-100">
				<div class="kard-option">
					<button class="btn text-muted" onclick="" ><i class="fas fa-chevron-down"></i></button>
					<div class="dropdown-list" style="display:none">
						<span onclick="commentDelete(<?php e($row['bsc_id']) ?>)" class="dropdown-item">Delete</span>
					</div>
				</div>
				...
				<div class="bg-light p-2">
					...
					<br>
					<?php parsedown($row['bsc_content']) ?>
				</div>
				<span class="text-muted small btn btn-sm"><?php e( format_datetime($row['bsc_created_date'])) ?></span>
				<a class="btn btn-link btn-sm" href="<?php e(home().'comment-buynsell/'.$row['bsc_id']) ?>"><?php e($row['bsc_count_reply']) ?> balasan</a>
			</div>
		</div>
		<?php
			}
	...
	<div class="popup" id="popup-form" style="display:none">
		...
		<form class="kard" id="comment-create" style="display:none">
			<h6>Buat komentar</h6>
			<label>Konten</label>
			<textarea class="form-control" name="bsc_content"></textarea>
			<button class="btn btn-primary btn-sm">Submit</button>
			<button type="button" class="btn btn-outline-secondary btn-sm ml-2 cancel-form">Batal</button>
		</form>
...
function _script($data) {
	...
	foreach ($data['comments']['list'] as $key => $value) {
		$list_comment[$value['bsc_id']] = $value;
	}
